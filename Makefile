...
testpy: ## run python unit tests
	python -m pytest -v jupyter_templates/tests --junitxml=junit.xml --cov=jupyter_templates --cov-report=xml:.coverage.xml --cov-branch --cov-fail-under=50 --cov-report term-missing
...
fixpy:  ## format python with ruff
	python -m ruff format jupyter_templates setup.py
...
clean: ## clean the repository
	...
	rm -rf jupyter_templates/labextension
	cd js && yarn clean
	git clean -fd
